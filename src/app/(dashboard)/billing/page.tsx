...
export default function BillingPage() {
  ...
  const paymentOptions = [
    ...
    { value: 'wave',         label: t('paymentWaveSoon'),   disabled: true },
    { value: 'orange_money', label: t('paymentOrangeSoon'), disabled: true },
  ]
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-4 md:p-6 space-y-4">
        ...
        <div className="rounded-xl border border-blue-100 bg-blue-50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-blue-900">{t('onlinePaymentsTitle')}</p>
              <p className="text-xs text-blue-700 mt-0.5">{t('onlinePaymentsDesc')}</p>
            </div>
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-4">
          {[
            { label: t('summaryCollected'), value: formatCurrency(totalRevenue), color: 'text-emerald-700', bg: 'bg-emerald-50' },
            { label: t('summaryPending'),   value: formatCurrency(totalPending), color: 'text-amber-700',   bg: 'bg-amber-50' },
            { label: t('summaryInvoices'),  value: invoices?.length ?? 0,        color: 'text-blue-700',    bg: 'bg-blue-50' },
          ].map(s => (
          ...
          <CardContent className="p-0">
            ...
            {!isLoading && (!invoices || invoices.length === 0) && (
              <EmptyState
                icon={Receipt}
                title={t('emptyTitle')}
                description={statusFilter !== 'all' ? t('emptyFilterDesc') : t('emptyDesc')}
                action={statusFilter === 'all' ? { label: t('emptyAction'), onClick: openCreate } : undefined}
              />
            )}
                  ...
                  <div key={inv.id} className="p-4 space-y-2">
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      ...
                      {Number(inv.amount_paid) > 0 && (
                        <span className="text-emerald-600">{t('paidBadge', { amount: formatCurrency(Number(inv.amount_paid)) })}</span>
                      )}
                    </div>
                      ...
                      {unpaid && (
                        <>
                          ...
                          <Button size="sm" className="flex-1 h-9 text-xs bg-emerald-600 hover:bg-emerald-700"
                            ...
                            <CheckCircle2 className="h-3.5 w-3.5 mr-1" /> {t('mobilePaid')}
                          </Button>
                        </>
                      )}
                    </div>
                          ...
                          {inv.status !== 'paid' && inv.status !== 'cancelled' && (
                            <>
                              ...
                              <Button size="sm" variant="outline"
                                ...
                                onClick={() => markPaid(inv)}>
                                <CheckCircle2 className="h-3 w-3 mr-1" /> {t('markPaidBtn')}
                              </Button>
                            </>
                          )}
                        </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Button type="button" size="sm" variant="outline"
                  ...
                  <Plus className="h-3 w-3" /> {t('addLineItem')}
                </Button>
              </div>
                ...
                {fields.map((field, idx) => (
                  <div key={field.id} className="grid grid-cols-12 gap-2 items-center">
                    <div className="col-span-5">
                      <Input placeholder={t('descriptionPlaceholder')} {...register(`line_items.${idx}.description`)} className="h-8 text-sm" />
                    </div>
                    <div className="col-span-2">
                      <Input type="number" placeholder={t('qtyPlaceholder')}
                        {...register(`line_items.${idx}.quantity`, { valueAsNumber: true })} className="h-8 text-sm" />
                    </div>
                    <div className="col-span-3">
                      <Input type="number" placeholder={t('unitPricePlaceholder')}
      ...
      {receiptInvoice && (
        <Dialog open onOpenChange={() => setReceiptInvoice(null)}>
          <DialogContent className="sm:max-w-md">
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setReceiptInvoice(null)}>{t('close')}</Button>
        ...
        <Dialog open onOpenChange={() => setPartialInvoice(null)}>
          <DialogContent className="sm:max-w-sm">
            ...
            <form onSubmit={partialForm.handleSubmit(onPartialPayment)} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <Label>{t('partialAmountLabel')}</Label>
                <Input
                  type="number"
                  placeholder="0"
              ...
              <div className="space-y-1.5">
                <Label>{t('partialMethodLabel')}</Label>
              ...
              <DialogFooter>
                <Button type="button" variant="outline" onClick={() => setPartialInvoice(null)}>{t('cancel')}</Button>

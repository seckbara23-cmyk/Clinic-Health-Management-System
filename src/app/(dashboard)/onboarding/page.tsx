...
export default function OnboardingPage() {
  ...
  const STEPS = [
    { id: 1, label: t('step1Label'), icon: Building2,    title: t('step1Title'), desc: t('step1Desc') },
    { id: 2, label: t('step2Label'), icon: Wrench,       title: t('step2Title'), desc: t('step2Desc') },
    { id: 3, label: t('step3Label'), icon: Users,        title: t('step3Title'), desc: t('step3Desc') },
    { id: 4, label: t('step4Label'), icon: CheckCircle2, title: t('step4Title'), desc: t('step4Desc') },
  ]
...
function StepProfile({
  ...
  async function save() {
    if (!clinic || !name.trim() || !location.trim()) {
      toast.error(t('profileErrorRequired'))
      return
    }
    setSaving(true)
...
function StepServices({ onNext }: { onNext: () => void }) {
  ...
  async function addCustom() {
    if (!name.trim()) { toast.error(t('servicesNameError')); return }
    const p = parseFloat(price)
    if (isNaN(p) || p < 0) { toast.error(t('servicesPriceError')); return }
    ...
    toast.success(t('servicesAddedToast'))
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Suggestions */}
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">{t('servicesSection')}</p>
      ...
      {showForm ? (
        ...
        >
          <Plus className="h-4 w-4" /> {t('servicesAddCustomBtn')}
        </button>
      )}
      ...
      >
        {services && services.length > 0 ? (
          <><ChevronRight className="h-4 w-4" /> {t('servicesContinueBtn')}</>
        ) : (
          t('servicesSkipBtn')
        )}
      </Button>
...
function StepTeam({
  ...
  async function sendInvite() {
    ...
    setEmail('')
    toast.success(t('teamInvitedToast', { email: emailTrimmed }))
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {sent.length > 0 && (
        <Card>
          <CardContent className="p-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">{t('teamSentSection')}</p>
            <div className="space-y-2">
              {sent.map((inv, i) => (
                <div key={i} className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4 text-emerald-500 shrink-0" />
                  <span className="text-sm text-gray-700">{inv.email}</span>
                  <span className="ml-auto text-xs text-gray-400">{roleLabels[inv.role] ?? inv.role}</span>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      >
        {sent.length > 0 ? (
          <><ChevronRight className="h-4 w-4" /> {t('teamContinueBtn')}</>
        ) : (
          t('teamSkipBtn')
        )}
      </Button>
  ...
  return (
    <Card>
      <CardContent className="p-8 text-center space-y-6">
        ...
        <div>
          <h3 className="text-xl font-bold text-gray-900">{t('finishTitle')}</h3>

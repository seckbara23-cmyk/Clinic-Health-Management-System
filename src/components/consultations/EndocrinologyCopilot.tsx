...
// ── Endocrinology Clinical Copilot — panel (Phase 33) ──────────────
//
// Sixteenth production Copilot, focused on endocrinology workflow, continuity and
// coordination. READ-ONLY intelligence (brief, endocrinology event tracker,
// laboratory / imaging workflow, operational reminders, completeness, medication
// review) plus clinician-initiated event data entry (recording an endocrinology
// event / laboratory or imaging order and advancing its status). It is NOT a
// clinical decision system: it NEVER diagnoses, classifies diabetes / thyroid /
// pituitary / adrenal disease, interprets HbA1c / hormones / MRI / ultrasound,
// recommends insulin / a medication / a dosage / surgery / admission / discharge,
// or predicts complications / mortality / cardiovascular risk — it only surfaces
...
import { useEndocrinologyEvents, useRecordEndocrinologyEvent, useUpdateEndocrinologyEventStatus, type EndoEventRow } from '@/hooks/useEndocrinology'
import {
  isEndocrinologyContext, countEndoImagingSignals, computeEndoCompleteness, buildEndoBrief,
  buildFollowUps, buildMedicationReview, ENDO_ALL_TYPES, ENDO_COPILOT_PACK_ID, categoryOf,
} from '@/lib/endocrinology/engine'
import { ENDO_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function EndocrinologyCopilot(props: Props) {
  const t = useTranslations('endocrinologyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isEndocrinologyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useEndocrinologyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordEndocrinologyEvent()
  const advanceEvent = useUpdateEndocrinologyEventStatus()

  const now = useMemo(() => new Date(), [])

  const imagingSignals = useMemo(() => countEndoImagingSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeEndoCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildEndoBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('endocrinology_consultation')
  ...
  function onAdvance(e: EndoEventRow) {
    ...
    <Card className="border-orange-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-orange-600 text-white"><Droplet className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-orange-600">{ENDO_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            {s.diabetesFollowups > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumDiabetes', { count: s.diabetesFollowups })}</Badge>}
            <Badge variant="outline">{t('sumThyroid', { count: s.thyroidFollowups })}</Badge>
            <Badge variant="outline">{t('sumLabs', { count: s.pendingLabs })}</Badge>
            <Badge variant="outline">{t('sumImaging', { count: s.pendingImaging })}</Badge>
            {s.nutrition > 0 && <Badge variant="outline">{t('sumNutrition', { count: s.nutrition })}</Badge>}
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-orange-300')} />
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-orange-400' : 'bg-gray-300')} />
          ...
          {/* Record a new endocrinology event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{ENDO_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        {(testRows.length > 0 || imagingSignals.investigations > 0) && (
          <Section icon={FlaskConical} title={t('labTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {testRows.map(r => (
        ...
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-orange-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {ENDO_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>
  ...
  return (
    <div>
      <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-gray-700">
        <Icon className="h-3.5 w-3.5 text-orange-600" /> {title}

...
// ── Ophthalmology Clinical Copilot — panel (Phase 26) ──────────────
//
// Ninth production Copilot, focused on eye-care workflow. READ-ONLY intelligence
// (brief, ophthalmology event tracker, imaging/test follow-up, operational
// reminders, completeness, medication review) plus clinician-initiated event
// data entry (recording an eye event / imaging order and advancing its status).
...
// medication, or predicts vision loss — it only surfaces that an event or an
// image exists and its workflow status. Extends and reuses the GP engine — no
// duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  Eye, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, Scan, Stethoscope,
} from 'lucide-react'
...
import { useOphthalmologyEvents, useRecordOphthalmologyEvent, useUpdateOphthalmologyEventStatus, type OphthEventRow } from '@/hooks/useOphthalmology'
import {
  isOphthalmologyContext, countOphthImagingSignals, computeOphthCompleteness, buildOphthBrief,
  buildFollowUps, buildMedicationReview, OPHTH_ALL_TYPES, OPHTH_COPILOT_PACK_ID, categoryOf,
} from '@/lib/ophthalmology/engine'
import { OPHTH_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function OphthalmologyCopilot(props: Props) {
  const t = useTranslations('ophthalmologyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isOphthalmologyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useOphthalmologyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordOphthalmologyEvent()
  const advanceEvent = useUpdateOphthalmologyEventStatus()

  const now = useMemo(() => new Date(), [])

  const imagingSignals = useMemo(() => countOphthImagingSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeOphthCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildOphthBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('visual_acuity')
  ...
  function onAdvance(e: OphthEventRow) {
  ...
  const imagingRows = brief.imaging.filter(r => r.total > 0 || r.reviewed > 0)
    ...
    <Card className="border-cyan-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-cyan-600 text-white"><Eye className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-cyan-600">{OPHTH_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            {s.awaitingImaging > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumAwaiting', { count: s.awaitingImaging })}</Badge>}
            <Badge variant="outline">{t('sumCataractGlaucoma', { count: s.cataractGlaucoma })}</Badge>
            <Badge variant="outline">{t('sumDiabetic', { count: s.diabeticScreening })}</Badge>
            <Badge variant="outline">{t('sumProcedures', { count: s.recentProcedures })}</Badge>
          </div>
        </Section>
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-cyan-300')} />
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-cyan-400' : 'bg-gray-300')} />
          ...
          {/* Record a new ophthalmology event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{OPHTH_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        {(imagingRows.length > 0 || imagingSignals.imaging > 0) && (
          <Section icon={Scan} title={t('imagingTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {imagingRows.map(r => (
                <Badge key={r.imagingType} variant="outline" className={r.awaitingReview > 0 ? 'border-amber-300 text-amber-700' : ''}>
                  {t(`evt_${r.imagingType}`)}: {r.total}{r.awaitingReview > 0 ? ` · ${r.awaitingReview}${t('awaitingShort')}` : ''}
                </Badge>
              ))}
              {imagingSignals.imaging > 0 && <Badge variant="outline">{t('sigImaging', { count: imagingSignals.imaging })}</Badge>}
            ...
            <p className="mt-1 text-[10px] text-gray-400">{t('imagingNoInterpret')}</p>
          </Section>
        )}

        {/* Documentation completeness */}
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-cyan-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {OPHTH_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>
  ...
  return (
    <div>
      <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-gray-700">
        <Icon className="h-3.5 w-3.5 text-cyan-600" /> {title}

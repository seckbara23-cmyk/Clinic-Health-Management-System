...
// ── Pulmonology Clinical Copilot — panel (Phase 28) ────────────────
//
// Eleventh production Copilot, focused on respiratory workflow. READ-ONLY
// intelligence (brief, respiratory event tracker, imaging/test workflow,
// operational reminders, completeness, medication review) plus clinician-
// initiated event data entry (recording a respiratory event / test order and
// advancing its status). It NEVER diagnoses, interprets an investigation
// (spirometry / PFT / chest imaging / bronchoscopy / sleep study), classifies
// COPD / asthma / fibrosis / pneumonia, recommends a treatment or medication, or
// predicts deterioration — it only surfaces that an event or a test exists and
// its workflow status. Extends and reuses the GP engine — no duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  Wind, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, Scan, Stethoscope,
} from 'lucide-react'
...
import { usePulmonologyEvents, useRecordPulmonologyEvent, useUpdatePulmonologyEventStatus, type PulmEventRow } from '@/hooks/usePulmonology'
import {
  isPulmonologyContext, countPulmImagingSignals, computePulmCompleteness, buildPulmBrief,
  buildFollowUps, buildMedicationReview, PULM_ALL_TYPES, PULM_COPILOT_PACK_ID, categoryOf,
} from '@/lib/pulmonology/engine'
import { PULM_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function PulmonologyCopilot(props: Props) {
  const t = useTranslations('pulmonologyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isPulmonologyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = usePulmonologyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordPulmonologyEvent()
  const advanceEvent = useUpdatePulmonologyEventStatus()

  const now = useMemo(() => new Date(), [])

  const imagingSignals = useMemo(() => countPulmImagingSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computePulmCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildPulmBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('pulmonary_consultation')
  ...
  function onAdvance(e: PulmEventRow) {
    ...
    <Card className="border-sky-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-sky-600 text-white"><Wind className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-sky-600">{PULM_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            <Badge variant="outline">{t('sumTests', { count: s.pendingTests })}</Badge>
            {s.awaitingTests > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumAwaiting', { count: s.awaitingTests })}</Badge>}
            <Badge variant="outline">{t('sumBronchoscopy', { count: s.bronchoscopy })}</Badge>
            <Badge variant="outline">{t('sumOxygen', { count: s.oxygenFollowUp })}</Badge>
            <Badge variant="outline">{t('sumRehab', { count: s.rehabReferrals })}</Badge>
          </div>
        </Section>
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
          ...
          {/* Record a new pulmonology event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{PULM_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        {(testRows.length > 0 || imagingSignals.imaging > 0) && (
          <Section icon={Scan} title={t('testsTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {testRows.map(r => (
              ...
              {imagingSignals.imaging > 0 && <Badge variant="outline">{t('sigTests', { count: imagingSignals.imaging })}</Badge>}
            ...
            <p className="mt-1 text-[10px] text-gray-400">{t('testsNoInterpret')}</p>
          </Section>
        )}
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {PULM_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>

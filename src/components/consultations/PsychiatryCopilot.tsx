...
// ── Psychiatry / Mental Health Clinical Copilot — panel (Phase 27) ─
//
// Tenth production Copilot, focused on workflow, continuity, documentation
// completeness and follow-up. READ-ONLY intelligence (brief, event tracker,
// operational reminders, safety-plan documentation PRESENCE, completeness,
// medication review) plus clinician-initiated event data entry (recording a
// mental-health workflow event and advancing its status). It NEVER diagnoses,
// classifies a psychiatric condition, predicts risk / self-harm / suicide,
// recommends treatment / medication / involuntary admission, or generates any
// sensitive conclusion — it only surfaces that a workflow event exists and its
// status. Extends and reuses the GP engine — no duplication.
//
// Confidentiality: this panel renders only in the treating psychiatrist's
// consultation context; the underlying events are RLS-restricted to the care
// team (no super_admin, no non-clinical roles).

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  Brain, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, ShieldCheck, Stethoscope,
} from 'lucide-react'
...
import { usePsychiatryEvents, useRecordPsychiatryEvent, useUpdatePsychiatryEventStatus, type MhEventRow } from '@/hooks/usePsychiatry'
import {
  isPsychiatryContext, computeMhCompleteness, buildMhBrief,
  buildFollowUps, buildMedicationReview, MH_EVENT_TYPES, MH_COPILOT_PACK_ID, categoryOf,
} from '@/lib/psychiatry/engine'
import { MH_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
const ADVANCEABLE = new Set(['planned', 'active', 'awaiting_review', 'follow_up_due'])
const RECORD_STATUSES = ['planned', 'active', 'awaiting_review', 'follow_up_due']
...
export function PsychiatryCopilot(props: Props) {
  const t = useTranslations('psychiatryCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isPsychiatryContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = usePsychiatryEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordPsychiatryEvent()
  const advanceEvent = useUpdatePsychiatryEventStatus()
  ...
  const completeness = useMemo(() => computeMhCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildMhBrief({
    now,
    ...
    lastConsultationAt, events: events ?? [], followUps,
    loaded: { prescriptions: !!props.prescriptions, labs: !!labs, invoices: !!props.invoices },
  }), [props, now, labs, appts, lastConsultationAt, events, followUps])

  const openEvents = useMemo(() => (events ?? []).filter(e => ADVANCEABLE.has(e.status)), [events])

  const [newType, setNewType] = useState<string>('initial_assessment')
  ...
  function onAdvance(e: MhEventRow) {
    ...
    <Card className="border-violet-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-violet-600 text-white"><Brain className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-violet-600">{MH_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            <Badge variant="outline">{t('sumEvents', { count: s.recentEvents })}</Badge>
            <Badge variant="outline">{t('sumSessions', { count: s.sessions })}</Badge>
            <Badge variant="outline">{t('sumReferrals', { count: s.referralFollowUps })}</Badge>
            {s.crisisFollowUps > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumCrisis', { count: s.crisisFollowUps })}</Badge>}
            <Badge variant="outline" className={s.safetyPlanDocumented ? 'border-emerald-300 text-emerald-700' : 'border-amber-300 text-amber-700'}>
              <ShieldCheck className="mr-1 h-3 w-3" />{s.safetyPlanDocumented ? t('safetyPlanYes') : t('safetyPlanNo')}
            </Badge>
          </div>
        </Section>

        {/* Mental-health event tracker (counts only — never interpret) */}
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-violet-300')} />
                  <span className="text-gray-700">
                    {t(r.labelKey, { ...(r.params ?? {}), ...(r.eventType ? { event: t(`evt_${r.eventType}`) } : {}) } as Record<string, string | number>)}
                  </span>
                </li>
              ))}
            </ul>
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-violet-400' : 'bg-gray-300')} />
          ...
          {/* Record a new mental-health event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{MH_EVENT_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-violet-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {MH_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>
  ...
  return (
    <div>
      <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-gray-700">
        <Icon className="h-3.5 w-3.5 text-violet-600" /> {title}

...
// ── Urology Clinical Copilot — panel (Phase 35) ────────────────────
//
// Eighteenth production Copilot, focused on urology workflow, continuity and
// coordination. READ-ONLY intelligence (brief, urology event tracker,
// investigation workflow, operational reminders, completeness, medication review)
// plus clinician-initiated event data entry (recording a urology event /
// investigation order and advancing its status). It is NOT a clinical decision
// system: it NEVER diagnoses, interprets laboratory / ultrasound / CT / MRI /
// cystoscopy, classifies kidney stones / prostate / bladder disease / urinary
// infections, recommends a medication / antibiotic / surgery / procedure /
// catheterization / admission / discharge / dialysis, or predicts renal outcome /
// cancer — it only surfaces that an event or a test exists and its workflow status.
// Extends and reuses the GP engine — no duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  TestTube, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, FlaskConical, Stethoscope,
} from 'lucide-react'
...
import { useUrologyEvents, useRecordUrologyEvent, useUpdateUrologyEventStatus, type UroEventRow } from '@/hooks/useUrology'
import {
  isUrologyContext, countUroImagingSignals, computeUroCompleteness, buildUroBrief,
  buildFollowUps, buildMedicationReview, URO_ALL_TYPES, URO_COPILOT_PACK_ID, categoryOf,
} from '@/lib/urology/engine'
import { URO_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function UrologyCopilot(props: Props) {
  const t = useTranslations('urologyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isUrologyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useUrologyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordUrologyEvent()
  const advanceEvent = useUpdateUrologyEventStatus()

  const now = useMemo(() => new Date(), [])

  const imagingSignals = useMemo(() => countUroImagingSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeUroCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildUroBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('urology_consultation')
  ...
  function onAdvance(e: UroEventRow) {
    ...
    <Card className="border-blue-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-600 text-white"><TestTube className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-blue-600">{URO_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            {s.pendingProcedures > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumProcedures', { count: s.pendingProcedures })}</Badge>}
            {s.catheterCare > 0 && <Badge variant="outline">{t('sumCatheter', { count: s.catheterCare })}</Badge>}
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-blue-300')} />
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-blue-400' : 'bg-gray-300')} />
          ...
          {/* Record a new urology event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{URO_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        {(testRows.length > 0 || imagingSignals.investigations > 0) && (
          <Section icon={FlaskConical} title={t('investTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {testRows.map(r => (
        ...
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-blue-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {URO_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>
  ...
  return (
    <div>
      <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-gray-700">
        <Icon className="h-3.5 w-3.5 text-blue-600" /> {title}

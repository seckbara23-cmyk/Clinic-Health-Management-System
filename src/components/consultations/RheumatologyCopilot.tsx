...
// ── Rheumatology Clinical Copilot — panel (Phase 38) ───────────────
//
// Twenty-first production Copilot, focused on rheumatology workflow, continuity and
// coordination. READ-ONLY intelligence (brief, rheumatology event tracker,
// investigation workflow, operational reminders, completeness, medication review)
// plus clinician-initiated event data entry (recording a rheumatology event /
// investigation order and advancing its status). It is NOT a clinical decision
// system: it NEVER diagnoses, interprets laboratory (ANA / RF / anti-CCP / ESR /
// CRP / HLA-B27) / joint aspiration / X-ray / ultrasound / MRI, classifies
// rheumatologic conditions, recommends DMARDs / biologics / steroids / NSAIDs /
// surgery / injections, or predicts disability / progression — it only surfaces
...
import {
  Bone, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, Microscope, Stethoscope,
} from 'lucide-react'
...
import { useRheumatologyEvents, useRecordRheumatologyEvent, useUpdateRheumatologyEventStatus, type RheumEventRow } from '@/hooks/useRheumatology'
import {
  isRheumatologyContext, countRheumImagingSignals, computeRheumCompleteness, buildRheumBrief,
  buildFollowUps, buildMedicationReview, RHEUM_ALL_TYPES, RHEUM_COPILOT_PACK_ID, categoryOf,
} from '@/lib/rheumatology/engine'
import { RHEUM_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function RheumatologyCopilot(props: Props) {
  const t = useTranslations('rheumatologyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isRheumatologyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useRheumatologyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordRheumatologyEvent()
  const advanceEvent = useUpdateRheumatologyEventStatus()

  const now = useMemo(() => new Date(), [])

  const imagingSignals = useMemo(() => countRheumImagingSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeRheumCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildRheumBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('rheumatology_consultation')
  ...
  function onAdvance(e: RheumEventRow) {
    ...
    <Card className="border-stone-200">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-stone-600 text-white"><Bone className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-stone-600">{RHEUM_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            <Badge variant="outline">{t('sumSerology', { count: s.pendingSerology })}</Badge>
            <Badge variant="outline">{t('sumImaging', { count: s.pendingImaging })}</Badge>
            {s.pendingAspiration > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumAspiration', { count: s.pendingAspiration })}</Badge>}
            <Badge variant="outline">{t('sumInfusion', { count: s.infusionMonitoring })}</Badge>
            {s.rehab > 0 && <Badge variant="outline">{t('sumRehab', { count: s.rehab })}</Badge>}
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-stone-300')} />
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-stone-400' : 'bg-gray-300')} />
          ...
          {/* Record a new rheumatology event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{RHEUM_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-stone-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {RHEUM_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>
  ...
  return (
    <div>
      <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-gray-700">
        <Icon className="h-3.5 w-3.5 text-stone-600" /> {title}

...
// ── Nephrology Clinical Copilot — panel (Phase 29) ─────────────────
//
// Twelfth production Copilot, focused on kidney-care workflow. READ-ONLY
// intelligence (brief, nephrology event tracker, laboratory/imaging workflow,
// operational reminders, completeness, medication review) plus clinician-
// initiated event data entry (recording a kidney-care event / lab / imaging
// order and advancing its status). It NEVER diagnoses, classifies a CKD stage /
// AKI / nephrotic syndrome, interprets a laboratory value, recommends dialysis /
// a treatment / a medication, or predicts renal failure — it only surfaces that
// an event or a test exists and its workflow status. Extends and reuses the GP
// engine — no duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  Droplet, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, FlaskConical, Stethoscope,
} from 'lucide-react'
...
import { useNephrologyEvents, useRecordNephrologyEvent, useUpdateNephrologyEventStatus, type NephroEventRow } from '@/hooks/useNephrology'
import {
  isNephrologyContext, countNephroLabSignals, computeNephroCompleteness, buildNephroBrief,
  buildFollowUps, buildMedicationReview, NEPHRO_ALL_TYPES, NEPHRO_COPILOT_PACK_ID, categoryOf,
} from '@/lib/nephrology/engine'
import { NEPHRO_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function NephrologyCopilot(props: Props) {
  const t = useTranslations('nephrologyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isNephrologyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useNephrologyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordNephrologyEvent()
  const advanceEvent = useUpdateNephrologyEventStatus()

  const now = useMemo(() => new Date(), [])

  const labSignals = useMemo(() => countNephroLabSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeNephroCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildNephroBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('nephrology_consultation')
  ...
  function onAdvance(e: NephroEventRow) {
    ...
    <Card className="border-teal-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-teal-600 text-white"><Droplet className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-teal-600">{NEPHRO_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            <Badge variant="outline">{t('sumDialysis', { count: s.dialysis })}</Badge>
            <Badge variant="outline">{t('sumLabs', { count: s.pendingLabs })}</Badge>
            {s.biopsy > 0 && <Badge variant="outline">{t('sumBiopsy', { count: s.biopsy })}</Badge>}
            {s.transplant > 0 && <Badge variant="outline">{t('sumTransplant', { count: s.transplant })}</Badge>}
            <Badge variant="outline">{t('sumNutrition', { count: s.nutritionReferral })}</Badge>
          </div>
        </Section>
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-teal-300')} />
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-teal-400' : 'bg-gray-300')} />
          ...
          {/* Record a new nephrology event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{NEPHRO_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        {(testRows.length > 0 || labSignals.renal > 0) && (
          <Section icon={FlaskConical} title={t('labsTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {testRows.map(r => (
              ...
              {labSignals.renal > 0 && <Badge variant="outline">{t('sigRenal', { count: labSignals.renal })}</Badge>}
            ...
            <p className="mt-1 text-[10px] text-gray-400">{t('labsNoInterpret')}</p>
          </Section>
        )}
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {NEPHRO_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>
  ...
  return (
    <div>
      <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-gray-700">
        <Icon className="h-3.5 w-3.5 text-teal-600" /> {title}

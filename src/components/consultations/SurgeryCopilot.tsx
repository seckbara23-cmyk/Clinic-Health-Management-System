...
// ── General Surgery Clinical Copilot — panel (Phase 31) ────────────
//
// Fourteenth production Copilot, focused on surgical workflow, continuity and
// coordination. READ-ONLY intelligence (brief, surgical event tracker,
// investigation workflow, operational reminders, completeness, medication review)
// plus clinician-initiated event data entry (recording a surgical event /
// investigation order and advancing its status). It is NOT a clinical decision
// system: it NEVER diagnoses, recommends surgery / conservative management / a
// medication / admission / discharge / ICU / an operative technique / anaesthesia
// / a transfusion, interprets pathology / CT / MRI / X-ray / endoscopy, or
// predicts complications / mortality / surgical success — it only surfaces that an
// event or an investigation exists and its workflow status. Extends and reuses the
// GP engine — no duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  Scissors, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, Microscope, Stethoscope,
} from 'lucide-react'
...
import { useSurgeryEvents, useRecordSurgeryEvent, useUpdateSurgeryEventStatus, type SurgeryEventRow } from '@/hooks/useSurgery'
import {
  isSurgeryContext, countSurgeryImagingSignals, computeSurgeryCompleteness, buildSurgeryBrief,
  buildFollowUps, buildMedicationReview, SURGERY_ALL_TYPES, SURGERY_COPILOT_PACK_ID, categoryOf,
} from '@/lib/surgery/engine'
import { SURGERY_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function SurgeryCopilot(props: Props) {
  const t = useTranslations('surgeryCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isSurgeryContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useSurgeryEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordSurgeryEvent()
  const advanceEvent = useUpdateSurgeryEventStatus()

  const now = useMemo(() => new Date(), [])

  const imagingSignals = useMemo(() => countSurgeryImagingSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeSurgeryCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildSurgeryBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('surgical_consultation')
  ...
  function onAdvance(e: SurgeryEventRow) {
    ...
    <Card className="border-slate-200">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-700 text-white"><Scissors className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-slate-600">{SURGERY_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            {s.scheduledSurgery > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumScheduled', { count: s.scheduledSurgery })}</Badge>}
            <Badge variant="outline">{t('sumPostop', { count: s.postopReviews })}</Badge>
            <Badge variant="outline">{t('sumWoundDrain', { count: s.woundDrain })}</Badge>
            <Badge variant="outline">{t('sumPathology', { count: s.pendingPathology })}</Badge>
            {s.icuWard > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumIcuWard', { count: s.icuWard })}</Badge>}
            {s.discharge > 0 && <Badge variant="outline">{t('sumDischarge', { count: s.discharge })}</Badge>}
          </div>
        </Section>
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-slate-300')} />
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-slate-400' : 'bg-gray-300')} />
          ...
          {/* Record a new surgery event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{SURGERY_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-slate-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {SURGERY_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>
  ...
  return (
    <div>
      <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-gray-700">
        <Icon className="h-3.5 w-3.5 text-slate-600" /> {title}

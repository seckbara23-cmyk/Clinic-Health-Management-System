...
// ── Emergency Medicine Clinical Copilot — panel (Phase 23) ─────────
//
// Sixth production Copilot. READ-ONLY intelligence (brief, pending-results
// tracker, observation tracker, procedure tracker, operational reminders, lab
// signals, completeness, medication review) plus clinician-initiated event data
// entry (recording an ED workflow event and advancing its status). It NEVER
// diagnoses, determines triage, recommends a treatment / medication / admission /
// discharge / procedure, or predicts deterioration / mortality — it only surfaces
// that an event exists and its workflow status. Extends and reuses the GP engine
// — no duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  Siren, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, Stethoscope, Eye, Syringe,
} from 'lucide-react'
...
import { useEmergencyEvents, useRecordEmergencyEvent, useUpdateEmergencyEventStatus, type EmergencyEventRow } from '@/hooks/useEmergency'
import {
  isEmergencyContext, countEmergencyLabSignals, computeEmergencyCompleteness, buildEmergencyBrief,
  buildFollowUps, buildMedicationReview, EMERGENCY_EVENT_TYPES, EMERGENCY_COPILOT_PACK_ID, categoryOf,
} from '@/lib/emergency/engine'
import { EMERGENCY_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
const ADVANCEABLE = new Set(['ordered', 'completed', 'awaiting_review', 'planned', 'performed', 'follow_up', 'started', 'ongoing'])
const RECORD_STATUSES = ['ordered', 'awaiting_review', 'planned', 'performed', 'follow_up', 'started', 'ongoing']
...
function nextStatus(s: string): string {
  switch (s) {
    ...
    case 'planned': return 'performed'
    case 'performed': return 'follow_up'
...
export function EmergencyCopilot(props: Props) {
  const t = useTranslations('emergencyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isEmergencyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useEmergencyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordEmergencyEvent()
  const advanceEvent = useUpdateEmergencyEventStatus()

  const now = useMemo(() => new Date(), [])

  const labSignals = useMemo(() => countEmergencyLabSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeEmergencyCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildEmergencyBrief({
    now,
  ...
  const arrival = useMemo(() => (events ?? []).find(e => e.eventType === 'arrival') ?? null, [events])

  const [newType, setNewType] = useState<string>('lab')
  ...
  function onAdvance(e: EmergencyEventRow) {
    advanceEvent.mutate({ id: e.id, status: nextStatus(e.status), patientId: props.patientId }, { onSuccess: () => toast.success(t('eventUpdated')) })
  }

  const resultRows = brief.pendingResults.filter(r => r.total > 0 || r.reviewed > 0)
  const procRows = brief.procedures.filter(p => p.total > 0)
  const obs = brief.observation
  const e = brief.emergency
    ...
    <Card className="border-red-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-red-600 text-white"><Siren className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-red-600">{EMERGENCY_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          {arrival?.eventDate && <p className="mt-1.5 text-[11px] text-gray-500">{t('arrival')}: <span className="font-medium">{arrival.eventDate}</span></p>}
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            <Badge variant="outline">{t('sumLabs', { count: e.pendingLabs })}</Badge>
            <Badge variant="outline">{t('sumImaging', { count: e.pendingImaging })}</Badge>
            <Badge variant="outline">{t('sumEcg', { count: e.pendingEcg })}</Badge>
            <Badge variant="outline">{t('sumProcedures', { count: e.pendingProcedures })}</Badge>
            <Badge variant="outline">{t('sumConsults', { count: e.consultRequests })}</Badge>
            <Badge variant="outline">{t('sumAdmissions', { count: e.admissions })}</Badge>
            {e.returnVisits > 1 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumReturn', { count: e.returnVisits })}</Badge>}
          </div>
        </Section>

        {/* Pending results tracker (counts only — never interpret) */}
        <Section icon={Activity} title={t('pendingTitle')}>
          {resultRows.length === 0 ? <p className="text-xs text-gray-400">{t('pendingEmpty')}</p> : (
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {resultRows.map(r => (
                <Badge key={r.resultType} variant="outline" className={r.awaitingReview > 0 ? 'border-amber-300 text-amber-700' : ''}>
                  {t(`evt_${r.resultType}`)}: {r.total}{r.awaitingReview > 0 ? ` · ${r.awaitingReview}${t('awaitingShort')}` : ''}
                </Badge>
              ))}
            </div>
          )}
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-red-300')} />
                  <span className="text-gray-700">
                    {t(r.labelKey, { ...(r.params ?? {}), ...(r.resultType ? { test: t(`evt_${r.resultType}`) } : {}) } as Record<string, string | number>)}
                  </span>
                </li>
              ))}
            </ul>
              ...
              {openEvents.slice(0, 6).map(ev => (
                <div key={ev.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(ev.eventType) ? 'bg-red-400' : 'bg-gray-300')} />
                    {t(`evt_${ev.eventType}`)} <span className="text-[10px] text-gray-400">{t(`st_${ev.status}`)}</span>
                  </span>
                  <Button size="sm" variant="outline" className="h-6 gap-1 px-2 text-[10px]" disabled={advanceEvent.isPending} onClick={() => onAdvance(ev)}>
          ...
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                ...
                <SelectContent>{EMERGENCY_EVENT_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <Select value={newStatus} onValueChange={setNewStatus}>
              <SelectTrigger className="h-7 w-32 text-xs"><SelectValue /></SelectTrigger>
              <SelectContent>{RECORD_STATUSES.map(s => <SelectItem key={s} value={s}>{t(`st_${s}`)}</SelectItem>)}</SelectContent>
            </Select>
        ...
        {(obs.started > 0 || obs.ongoing > 0 || obs.completed > 0 || obs.admissions > 0 || obs.discharges > 0 || obs.transfers > 0) && (
          <Section icon={Eye} title={t('observationTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {obs.ongoing > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('obsOngoing', { count: obs.ongoing })}</Badge>}
              {obs.started > 0 && <Badge variant="outline">{t('obsStarted', { count: obs.started })}</Badge>}
              {obs.completed > 0 && <Badge variant="outline">{t('obsCompleted', { count: obs.completed })}</Badge>}
              {obs.admissions > 0 && <Badge variant="outline">{t('obsAdmissions', { count: obs.admissions })}</Badge>}
              {obs.discharges > 0 && <Badge variant="outline">{t('obsDischarges', { count: obs.discharges })}</Badge>}
              {obs.transfers > 0 && <Badge variant="outline">{t('obsTransfers', { count: obs.transfers })}</Badge>}
            </div>
          </Section>
        )}
        ...
        {procRows.length > 0 && (
          <Section icon={Syringe} title={t('procedureTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {procRows.map(p => (
                <Badge key={p.procedureType} variant="outline" className={p.followUp > 0 ? 'border-amber-300 text-amber-700' : ''}>
                  {t(`evt_${p.procedureType}`)}: {p.total}{p.followUp > 0 ? ` · ${p.followUp}${t('followUpShort')}` : ''}
                </Badge>
              ))}
            </div>
          </Section>
        ...
        {/* Lab / imaging signals (supplementary, from existing lab orders) */}
        {(labSignals.pending > 0 || labSignals.imaging > 0) && (
          <Section icon={FileText} title={t('labSignalsTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {labSignals.pending > 0 && <Badge variant="outline">{t('sigPending', { count: labSignals.pending })}</Badge>}
              {labSignals.completed > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sigCompleted', { count: labSignals.completed })}</Badge>}
        ...
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-red-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {EMERGENCY_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>

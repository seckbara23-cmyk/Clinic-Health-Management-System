...
// ── Orthopedics Clinical Copilot — panel (Phase 25) ────────────────
//
// Eighth production Copilot, focused on musculoskeletal workflow. READ-ONLY
// intelligence (brief, orthopedic event tracker, imaging follow-up, operational
// reminders, completeness, medication review) plus clinician-initiated event
// data entry (recording an orthopedic event / imaging order and advancing its
// status). It NEVER diagnoses, classifies a fracture, interprets an X-ray / CT /
// MRI, recommends surgery / a treatment / a medication, or predicts healing /
// disability — it only surfaces that an event or an image exists and its workflow
// status. Extends and reuses the GP engine — no duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  Bone, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, Scan, Stethoscope,
} from 'lucide-react'
...
import { useOrthopedicEvents, useRecordOrthopedicEvent, useUpdateOrthopedicEventStatus, type OrthoEventRow } from '@/hooks/useOrthopedics'
import {
  isOrthopedicsContext, countOrthoImagingSignals, computeOrthoCompleteness, buildOrthoBrief,
  buildFollowUps, buildMedicationReview, ORTHO_ALL_TYPES, ORTHO_COPILOT_PACK_ID, categoryOf,
} from '@/lib/orthopedics/engine'
import { ORTHO_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function OrthopedicsCopilot(props: Props) {
  const t = useTranslations('orthopedicsCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isOrthopedicsContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useOrthopedicEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordOrthopedicEvent()
  const advanceEvent = useUpdateOrthopedicEventStatus()

  const now = useMemo(() => new Date(), [])

  const imagingSignals = useMemo(() => countOrthoImagingSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeOrthoCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildOrthoBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('fracture_followup')
  ...
  function onAdvance(e: OrthoEventRow) {
    ...
    <Card className="border-amber-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-amber-600 text-white"><Bone className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-amber-600">{ORTHO_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            <Badge variant="outline">{t('sumCasts', { count: s.activeCasts })}</Badge>
            <Badge variant="outline">{t('sumProcedures', { count: s.recentProcedures })}</Badge>
            <Badge variant="outline">{t('sumPhysio', { count: s.physiotherapyReferrals })}</Badge>
          </div>
        </Section>
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-gray-300')} />
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-amber-400' : 'bg-gray-300')} />
          ...
          {/* Record a new orthopedic event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{ORTHO_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-amber-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {ORTHO_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>

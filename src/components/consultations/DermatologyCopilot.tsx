...
// ── Dermatology Clinical Copilot — panel (Phase 34) ────────────────
//
// Seventeenth production Copilot, focused on dermatology workflow, continuity and
// coordination. READ-ONLY intelligence (brief, dermatology event tracker,
// investigation workflow, operational reminders, completeness, medication review)
// plus clinician-initiated event data entry (recording a dermatology event /
// investigation order and advancing its status). It is NOT a clinical decision
// system: it NEVER diagnoses skin disease, classifies melanoma / eczema /
// psoriasis / dermatitis, interprets dermoscopy / pathology / biopsy, recommends a
// biopsy / surgery / a medication / a topical / an antibiotic / an antifungal, or
// predicts malignancy / recurrence — it only surfaces that an event or a test
// exists and its workflow status. Extends and reuses the GP engine — no duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  Fingerprint, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, Microscope, Stethoscope,
} from 'lucide-react'
...
import { useDermatologyEvents, useRecordDermatologyEvent, useUpdateDermatologyEventStatus, type DermEventRow } from '@/hooks/useDermatology'
import {
  isDermatologyContext, countDermImagingSignals, computeDermCompleteness, buildDermBrief,
  buildFollowUps, buildMedicationReview, DERM_ALL_TYPES, DERM_COPILOT_PACK_ID, categoryOf,
} from '@/lib/dermatology/engine'
import { DERM_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
export function DermatologyCopilot(props: Props) {
  const t = useTranslations('dermatologyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isDermatologyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useDermatologyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordDermatologyEvent()
  const advanceEvent = useUpdateDermatologyEventStatus()

  const now = useMemo(() => new Date(), [])

  const imagingSignals = useMemo(() => countDermImagingSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeDermCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildDermBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('dermatology_consultation')
  ...
  function onAdvance(e: DermEventRow) {
    ...
    <Card className="border-fuchsia-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-fuchsia-600 text-white"><Fingerprint className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-fuchsia-600">{DERM_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            ...
            {s.pendingPathology > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('sumPathology', { count: s.pendingPathology })}</Badge>}
            <Badge variant="outline">{t('sumProcedures', { count: s.pendingProcedures })}</Badge>
            <Badge variant="outline">{t('sumWoundCare', { count: s.woundCare })}</Badge>
            {s.phototherapy > 0 && <Badge variant="outline">{t('sumPhototherapy', { count: s.phototherapy })}</Badge>}
            {s.patchTesting > 0 && <Badge variant="outline">{t('sumPatch', { count: s.patchTesting })}</Badge>}
        ...
        <Section icon={Activity} title={t('trackerTitle')}>
            ...
            <ul className="mt-2 space-y-1">
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  <span className={cn('h-1.5 w-1.5 shrink-0 rounded-full', r.severity === 'warning' ? 'bg-amber-400' : 'bg-fuchsia-300')} />
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-fuchsia-400' : 'bg-gray-300')} />
          ...
          {/* Record a new dermatology event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{DERM_ALL_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        <Section icon={FileText} title={t('completenessTitle')} right={<span className="text-xs font-bold text-fuchsia-700">{completeness.overall}%</span>}>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {DERM_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>
  ...
  return (
    <div>
      <p className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-gray-700">
        <Icon className="h-3.5 w-3.5 text-fuchsia-600" /> {title}

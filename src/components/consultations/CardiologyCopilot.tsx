...
// ── Cardiology Clinical Copilot — panel (Phase 22) ─────────────────
//
// Fifth production Copilot. READ-ONLY intelligence (brief, cardiac test
// tracking, procedure tracker, cardiac lab follow-up, operational reminders,
// completeness, medication review) plus clinician-initiated event data entry
// (recording an ECG / Echo / stress test / procedure event and advancing its
// workflow status). It NEVER diagnoses, interprets an ECG / Echo / any test,
// recommends a medication or procedure, or predicts risk / mortality — it only
// surfaces that an event exists and its workflow status. Extends and reuses the
// GP engine — no duplication.

import { useMemo, useState } from 'react'
import { useTranslations } from 'next-intl'
import {
  HeartPulse, ClipboardCheck, Activity, FileText, Pill, ShieldQuestion, Plus, Check, Loader2, Stethoscope, Syringe,
} from 'lucide-react'
...
import { useCardiologyEvents, useRecordCardiologyEvent, useUpdateCardiologyEventStatus, type CardioEventRow } from '@/hooks/useCardiology'
import {
  isCardiologyContext, countCardiacLabSignals, computeCardiologyCompleteness, buildCardiologyBrief,
  buildFollowUps, buildMedicationReview, CARDIO_EVENT_TYPES, CARDIO_COPILOT_PACK_ID, categoryOf,
} from '@/lib/cardiology/engine'
import { CARDIO_SMART_TEMPLATE_IDS } from '@/lib/templates/registry'
...
const ADVANCEABLE = new Set(['ordered', 'scheduled', 'planned', 'completed', 'awaiting_review', 'follow_up', 'due'])
const RECORD_STATUSES = ['ordered', 'scheduled', 'planned', 'awaiting_review', 'follow_up', 'due']
...
function nextStatus(s: string): string {
  switch (s) {
    ...
    case 'completed': return 'follow_up'
    case 'follow_up': return 'reviewed'
    default: return 'reviewed'
  }
}

export function CardiologyCopilot(props: Props) {
  const t = useTranslations('cardiologyCopilot')
  const { formatCurrency } = useFormatters()
  const identity = useProfessionalIdentity()
  const { ai } = useClinicConfig()

  const active = isCardiologyContext(identity.profession.id, identity.specialties.primary?.id ?? null)
  const aiOn = ai('patient_intelligence')

  const safety = useMedicationSafety()
  const { data: events } = useCardiologyEvents(props.patientId)
  const { data: labs } = useLabOrders(props.patientId)
  const { data: appts } = useAppointments(undefined, props.patientId)
  const recordEvent = useRecordCardiologyEvent()
  const advanceEvent = useUpdateCardiologyEventStatus()

  const now = useMemo(() => new Date(), [])

  const labSignals = useMemo(() => countCardiacLabSignals(labs), [labs])
  ...
  const completeness = useMemo(() => computeCardiologyCompleteness(props.doc), [props.doc])
  ...
  const brief = useMemo(() => buildCardiologyBrief({
    now,
  ...
  const [newType, setNewType] = useState<string>('ecg')
    ...
    <Card className="border-rose-100">
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 border-b pb-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-rose-600 text-white"><HeartPulse className="h-4 w-4" /></div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-900">{t('title')}</p>
            <p className="text-[11px] text-gray-400">{t('subtitle')}</p>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] text-rose-600">{CARDIO_COPILOT_PACK_ID}</Badge>
        </div>
        ...
        <Section icon={ClipboardCheck} title={t('briefTitle')}>
          ...
          {/* Cardiac summary counts (surface only) */}
          <div className="mt-2 flex flex-wrap gap-1.5 text-[11px]">
            <Badge variant="outline">{t('sumEcg', { count: c.ecg })}</Badge>
            <Badge variant="outline">{t('sumEcho', { count: c.echo })}</Badge>
            <Badge variant="outline">{t('sumLabs', { count: c.cardiacLabs })}</Badge>
            <Badge variant="outline">{t('sumProcedures', { count: c.procedures })}</Badge>
            <Badge variant="outline">{t('sumAdmissions', { count: c.admissions })}</Badge>
          </div>
        </Section>

        {/* Cardiac test tracking (counts only — never interpret) */}
        <Section icon={Activity} title={t('testTrackingTitle')}>
          {testRows.length === 0 ? <p className="text-xs text-gray-400">{t('testEmpty')}</p> : (
              ...
              {brief.followUp.reminders.map(r => (
                <li key={r.code} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-gray-700">
                    {t(r.labelKey, { ...(r.params ?? {}), ...(r.testType ? { test: t(`evt_${r.testType}`) } : {}) } as Record<string, string | number>)}
                  </span>
                </li>
              ))}
            </ul>
              ...
              {openEvents.slice(0, 6).map(e => (
                <div key={e.id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className={cn('h-1.5 w-1.5 rounded-full', categoryOf(e.eventType) ? 'bg-rose-400' : 'bg-gray-300')} />
          ...
          {/* Record a new cardiology event */}
          <div className="mt-2 flex items-end gap-1.5 rounded-lg border bg-white p-2">
            <div className="min-w-0 flex-1">
              <Select value={newType} onValueChange={setNewType}>
                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                <SelectContent>{CARDIO_EVENT_TYPES.map(ty => <SelectItem key={ty} value={ty}>{t(`evt_${ty}`)}</SelectItem>)}</SelectContent>
              </Select>
            </div>
        ...
        {(labSignals.troponin > 0 || labSignals.bnp > 0 || labSignals.lipid > 0 || labSignals.hba1c > 0 || labSignals.electrolytes > 0 || labSignals.creatinine > 0) && (
          <Section icon={FileText} title={t('labFollowUpTitle')}>
            <div className="flex flex-wrap gap-1.5 text-[11px]">
              {labSignals.troponin > 0 && <Badge variant="outline">{t('labTroponin', { count: labSignals.troponin })}</Badge>}
              {labSignals.bnp > 0 && <Badge variant="outline">{t('labBnp', { count: labSignals.bnp })}</Badge>}
              {labSignals.lipid > 0 && <Badge variant="outline">{t('labLipid', { count: labSignals.lipid })}</Badge>}
              {labSignals.hba1c > 0 && <Badge variant="outline">{t('labHba1c', { count: labSignals.hba1c })}</Badge>}
              {labSignals.electrolytes > 0 && <Badge variant="outline">{t('labElectrolytes', { count: labSignals.electrolytes })}</Badge>}
              {labSignals.creatinine > 0 && <Badge variant="outline">{t('labCreatinine', { count: labSignals.creatinine })}</Badge>}
              {labSignals.awaitingReview > 0 && <Badge variant="outline" className="border-amber-300 text-amber-700">{t('labAwaiting', { count: labSignals.awaitingReview })}</Badge>}
            </div>
        ...
        <Section icon={Stethoscope} title={t('templatesTitle')}>
          <div className="flex flex-wrap gap-1.5">
            {CARDIO_SMART_TEMPLATE_IDS.map(id => <Badge key={id} variant="outline" className="text-[10px] text-gray-500">{t(`tpl_${id}`)}</Badge>)}
          </div>
        </Section>

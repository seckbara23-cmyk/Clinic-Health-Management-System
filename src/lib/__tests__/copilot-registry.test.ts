...
const REQUIRED_PACKS = [
  ...
  'internal_medicine.core', 'orthopedics.core', 'dentistry.core', 'nursing.core', 'midwifery.core',
]
...
describe('Copilot Pack Registry — integrity', () => {
  ...
  it('placeholder cores are METADATA ONLY (empty future*); the shipped copilots are populated', () => {
    // Phases 16/17/18/19/22/23/24/25 populated GP / pediatrics / obstetrics / ENT /
    // cardiology / emergency / internal-medicine / orthopedics cores. Rest are placeholders.
    const REAL = new Set(['general_practice.core', 'pediatrics.core', 'obstetrics.core', 'ent.core', 'cardiology.core', 'emergency.core', 'internal_medicine.core', 'orthopedics.core'])
  ...
  it('lookups never throw', () => {
    ...
    expect(activePacks().length).toBe(14)
  })
})

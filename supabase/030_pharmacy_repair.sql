-- ════════════════════════════════════════════════════════════════
-- 030 REPAIR — Pharmacy Inventory + Dispensing (Phase 5B)
-- ════════════════════════════════════════════════════════════════
--
-- Safe, idempotent re-run of 030_pharmacy.sql. Use this when 030 was
-- only partially applied (e.g. it failed with
--   policy "inventory_select" for table "clinic_medication_inventory"
--   already exists
-- because the original used bare CREATE POLICY / ALTER PUBLICATION).
--
-- This script:
--   • CREATE TABLE IF NOT EXISTS for every table (no data loss)
--   • DROP POLICY IF EXISTS before each CREATE POLICY (re-runnable RLS)
--   • CREATE OR REPLACE for every function
--   • DROP TRIGGER IF EXISTS before each CREATE TRIGGER
--   • guards ALTER PUBLICATION so re-running can't error on dup table
--
-- It is byte-for-byte equivalent in end state to 030_pharmacy.sql.
-- Running it on a clean DB, a partial DB, or a fully-migrated DB all
-- converge to the same schema. Existing pharmacy rows are untouched.
...
DROP TRIGGER IF EXISTS trg_inventory_updated_at ON public.clinic_medication_inventory;
...
DROP TRIGGER IF EXISTS trg_batches_updated_at ON public.medication_batches;
...
DROP TRIGGER IF EXISTS trg_dispensings_updated_at ON public.medication_dispensings;
CREATE TRIGGER trg_dispensings_updated_at
...
DROP POLICY IF EXISTS "inventory_select" ON public.clinic_medication_inventory;
...
DROP POLICY IF EXISTS "inventory_insert" ON public.clinic_medication_inventory;
...
DROP POLICY IF EXISTS "inventory_update" ON public.clinic_medication_inventory;
CREATE POLICY "inventory_update" ON public.clinic_medication_inventory FOR UPDATE
  ...
  WITH CHECK (clinic_id = public.get_clinic_id() AND public.get_user_role() IN ('admin','pharmacist'));
DROP POLICY IF EXISTS "inventory_delete" ON public.clinic_medication_inventory;
...
DROP POLICY IF EXISTS "batches_select" ON public.medication_batches;
CREATE POLICY "batches_select" ON public.medication_batches FOR SELECT
  USING (clinic_id = public.get_clinic_id()
         AND public.get_user_role() IN ('admin','doctor','pharmacist')
         AND (deleted_at IS NULL OR public.get_user_role() = 'admin'));
DROP POLICY IF EXISTS "batches_delete" ON public.medication_batches;
...
DROP POLICY IF EXISTS "dispensings_select" ON public.medication_dispensings;
CREATE POLICY "dispensings_select" ON public.medication_dispensings FOR SELECT
  USING (clinic_id = public.get_clinic_id()
         AND public.get_user_role() IN ('admin','doctor','pharmacist')
         AND (deleted_at IS NULL OR public.get_user_role() = 'admin'));
DROP POLICY IF EXISTS "dispensings_delete" ON public.medication_dispensings;
...
-- ── Realtime (guarded: ADD TABLE errors if already a member) ──────
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime'
      AND schemaname = 'public' AND tablename = 'clinic_medication_inventory'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime'
      AND schemaname = 'public' AND tablename = 'medication_dispensings'
  ) THEN
